...
bool TcpServer::run()
{
	if (this->listen(QHostAddress::AnyIPv4, m_port))//来自任何主机的连接都监听
	{
...
void TcpServer::SocketDisconnected(int descriptor)
{
	for (int i = 0; i < m_tcpSocketConnectList.count(); ++i)
	{
		...
		//查找断开连接的socket
		if (item->socketDescriptor() == descriptor || itemDescriptor == -1)
		{
			m_tcpSocketConnectList.removeAt(i);//断开的socket从列表中移除
