...
{
	m_onlineMap.insert(QStringLiteral("0"), QStringLiteral("离线"));
	m_onlineMap.insert(QStringLiteral("1"), QStringLiteral("在线"));
	m_onlineMap.insert(QStringLiteral("2"), QStringLiteral("隐身"));
}
...
void QtQQ_Server::on_logoutBtn_clicked()
{
	...
	//检查员工QQ号是否输入
	if (!ui.logoutBtn->text().length())
	{
		QMessageBox::information(this, QStringLiteral("提示"),
			QStringLiteral("请输入员工QQ号"));
		ui.logoutBtn->setFocus();
		return;
	}
...
void QtQQ_Server::on_addBtn_clicked()
{
	...
	if (!strName.size())
	{
		QMessageBox::information(this,
			QString::fromLocal8Bit("提示"),
			QString::fromLocal8Bit("请输入员工的姓名"));
