	...
	if (!createJSFile(employeesIDList))
		{
			QMessageBox::information(this,
				...
				QString::fromLocal8Bit("js文件更新数据失败"));
		}
	}
...
void TalkWindowShell::initTcpScoket()
{
...
bool TalkWindowShell::createJSFile(QStringList & employeesList)
{
	...
	else
	{
		QMessageBox::information(this
			, QString::fromLocal8Bit("提示"),
			QString::fromLocal8Bit("写 msgtmpl.txt.js 失败 !"));
		return false;
	}
}
...
//文本数据包格式: 群聊标志 + 发消息员工QQ号 + 收信息员工QQ号(群QQ号) + 信息类型 + 数据长度 + 数据
//表情数据包格式: 群聊标志 + 发消息员工QQ号 + 收信息员工QQ号(群QQ号) + 信息类型 + 表情个数 + images + 数据
...
void TalkWindowShell::updateSendTcpMsg(QString & strData, int & msgType, QString fileName)
{
	...
	{
		//文件数据包格式: 群聊标志 + 发消息员工QQ号 + 收信息员工QQ号(群QQ号)
...
void TalkWindowShell::onEmotionBtnClicked(bool)
{
	...
	QPoint emotonPoint = this->mapToGlobal(QPoint(0, 0));//将当前控件的相对位置转换为屏幕的绝对位置
...
/****************************************************************************************************************************************
	数据包格式:
	文本数据包格式:群聊标志 + 发消息员工QQ号 + 收信息员工QQ号(群QQ号) + 信息类型(1) + 数据长度 + 数据
	表情数据包格式:群聊标志 + 发消息员工QQ号 + 收信息员工QQ号(群QQ号) + 信息类型(0) + 表情个数 + images + 表情名称
	文件数据包格式:群聊标志 + 发消息员工QQ号 + 收信息员工QQ号(群QQ号) + 信息类型(2) + 文件字节数 + bytes + 文件名 + data_begin + 文件数据

	群聊标志占1位,0表示单聊, 1表示群聊
	信息类型占1位,0表示表情信息,1表示文本信息,2表示文件信息

	QQ号占5位, 群QQ号占4位, 数据长度占5位, 表情名称占3位
	(注意: 当群聊标志位1时, 则数据包没有收信息员工QQ号, 而是收信息群QQ号
		   当群聊标志位0时, 则数据包没有收信息群QQ号, 而是收信息员工QQ号)

	群聊文本信息如: 1100012001100005Hello	表示QQ10001向群2001发送文本信息, 长度是5, 数据位Hello
	单聊图片信息如: 0100011000201images060	表示QQ10001向QQ10002发送1个表情60.png
	群聊文件信息如: 1100052000210bytestest.txtdata_beginhelloworld
											表示QQ10005向群2000发送文件信息,文件是test.txt,文件内容长度10,内容是helloworld

	群聊文件信息解析: 1 10001 2001 1 00005 hello
	单聊图片信息解析: 0 10001 10002 0 images 060
	群聊文件信息解析: 1 10005 2000 2 10 bytes yexy.txt data_begin helloworld
...
void TalkWindowShell::processPendingData()
{
	...
	while (m_udpReceiver->hasPendingDatagrams())
	{
		...
		const static int picture = 3;			//表情图片的宽度
		...
		QString strWindowID;	//群聊窗口ID,群聊则是群号, 单聊则是员工QQ号
		...
		else//单聊
		{
			...
			if (cMsgType == '1')//文本信息
			{
				...
				//文本信息长度
				msgLen = strData.mid(groupFlagWidth + employeeWidth
					+ employeeWidth + msgTypeWidth, msgLengthWidth).toInt();
				strMsg = strData.mid(groupFlagWidth + employeeWidth
					+ employeeWidth + msgTypeWidth + msgLengthWidth, msgLen);
			}
			else if (cMsgType == '0')//表情信息
			{
				msgType = 0;
			...
			else if (cMsgType == '2')//文件信息
			{
				...
				QString sender;
				int employeeID = strSendEmployeeID.toInt();
				QSqlQuery querySendName(QString("SELECT employee_name FROM tab_employees WHERE employeeID = %1")
					.arg(employeeID));
				querySendName.exec();

				if (querySendName.first())
				{
					sender = querySendName.value(0).toString();
				}
